...
for i in tqdm(range(train.shape[0])):
    ...
    cap = cv2.VideoCapture(
        'Videos/' + path + '/' + videoFile.split(' ')[0].split('/')[1])  # capturing the video from the given path
    # print(cap.grab())
...
base_model = tf.keras.applications.densenet.DenseNet169(include_top=False, weights='imagenet',
...
model = Sequential()
model.add(
    Dense(1024, activation='relu', input_shape=(36864,)))  # size filled according to X_train size after prediction
